...
import type { Role, UserPortalRole } from '@chemisttasker/shared-core';
import {
...
export const fetchUserRoleByEmail = async (rawEmail: string): Promise<UserPortalRole | null> => {
  ...
  return typeof role === 'string' ? (role as UserPortalRole) : null;
};
...
export const describeRoleMismatch = (
  ...
  if (!requiredUserRole || userRole === null || typeof userRole === 'undefined') return null;
  if (userRole === requiredUserRole) return null;
...
export const formatExistingUserRole = (userRole: UserPortalRole | null | undefined): string | null => {
  if (userRole === null || typeof userRole === 'undefined') return null;
