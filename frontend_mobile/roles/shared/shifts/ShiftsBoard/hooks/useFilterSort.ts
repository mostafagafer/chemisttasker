// useFilterSort - Exact filtering and sorting logic from web
// Complete filter/sort implementation for ShiftsBoard

import { useEffect, useMemo, useState } from 'react';
import { startOfDay, endOfDay, isEqual, isAfter, isBefore } from 'date-fns';
...
type UseFilterSortParams = {
    ...
    activeTab: 'browse' | 'saved' | 'interested' | 'rejected';
        ...
        result = result.filter((shift) => {
            ...
            if (filterConfig.dateRange.start || filterConfig.dateRange.end) {
                const startFilter = filterConfig.dateRange.start
                    ? startOfDay(new Date(filterConfig.dateRange.start))
                    : new Date('1970-01-01');
                const endFilter = filterConfig.dateRange.end
                    ? endOfDay(new Date(filterConfig.dateRange.end))
                    : new Date('2100-01-01');
                const matchesDate = hasOverlap((slot) => {
                    const slotDate = new Date(slot.date ?? '');
                    return (
                        isEqual(slotDate, startFilter) ||
                        isEqual(slotDate, endFilter) ||
                        (isAfter(slotDate, startFilter) && isBefore(slotDate, endFilter))
                    );
                });
